...
const isAndroidApp = !!(window as any).__RAYSPEAKI_ANDROID__;
const isTablet = isMobile && Math.min(screen.width, screen.height) >= 600;
...
function showApp() {
  ...
  // Show screen share on Electron, camera on mobile
  if (isElectron || isMobile) $('screen-btn').style.display = '';
  if (isMobile) {
    // Update button icon/label for camera
    const sb = $('screen-btn');
    sb.innerHTML = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg><span class="dock-btn-label">摄像头</span>`;
    sb.title = '摄像头';
  }
  if (isPhone) initMobileUI();
}
...
async function apiPost(path: string, body: unknown) {
  return fetch(path, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Session-Token': session }, body: JSON.stringify(body) });
}
async function apiGet(path: string) {
  return fetch(path, { headers: { 'X-Session-Token': session } });
}
async function apiPut(path: string, body: unknown) {
  return fetch(path, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-Session-Token': session }, body: JSON.stringify(body) });
}
async function apiDelete(path: string) {
  return fetch(path, { method: 'DELETE', headers: { 'X-Session-Token': session } });
}
...
async function setChannelPassword(id: string) {
  ...
  const pw = prompt(ch?.hasPassword ? '输入新密码 (留空则取消密码):' : '设置频道密码:', '');
  if (pw === null) return; // cancelled
  const res = await apiPut(`/api/channels/${id}`, { password: pw });
  if (!res.ok) { alert('设置密码失败'); return; }
  await loadChannels();
}
...
async function leaveCurrentChannel() {
  ...
  // Stop screen/camera share if active
  if (screenProducer) {
...
function showScreenShareView(consumer: Consumer, peerId: string) {
  ...
  label.textContent = `${peer?.identity ?? '用户'} 正在共享屏幕/摄像头`;
...
async function toggleScreenShare() {
  ...
  // Mobile: use camera instead of screen share
  if (isMobile) {
    try { await toggleCameraShare(); } catch (err) { console.error('[camera] share failed:', err); }
    return;
  }
...
function updateScreenBtn() {
  ...
  if (screenSharing) {
    ...
    btn.title = isMobile ? '关闭摄像头' : '停止共享';
  } else {
    btn.classList.remove('sharing');
    btn.title = isMobile ? '摄像头' : '共享屏幕';
  }
}

// ─── Camera sharing (mobile) ─────────────────────────────────────────────────
...
async function toggleCameraShare() {
  ...
  const settings = await showCameraPickerDialog();
  if (!settings) return;
  ...
  // Show self-preview PiP
  showCameraPiP(stream);

  // Bitrate
  const baseBitrate = settings.resolution === '1080p' ? 4000000 : settings.resolution === '720p' ? 2500000 : 1500000;

  screenProducer = await sendTransport.produce({
    track: stream.getVideoTracks()[0],
    appData: { source: 'screen' },
    encodings: [{ maxBitrate: baseBitrate }],
    codecOptions: { videoGoogleStartBitrate: 1000 },
  });
...
async function showCameraPickerDialog(): Promise<CameraSettings | null> {
  return new Promise(async (resolve) => {
    const overlay = $('cam-picker-overlay');
    const preview = $('cam-preview') as HTMLVideoElement;
    const select = $('cam-select') as HTMLSelectElement;
    ...
    select.innerHTML = cameras.map((c, i) => {
      const label = c.label || `摄像头 ${i + 1}`;
      return `<option value="${c.deviceId}">${esc(label)}</option>`;
    }).join('');
    ...
    async function startPreview() {
      ...
      try {
        previewStream = await navigator.mediaDevices.getUserMedia({
          video: { deviceId: select.value ? { exact: select.value } : undefined, width: { ideal: 640 }, height: { ideal: 480 } }
        });
        preview.srcObject = previewStream;
      } catch {}
    }
    await startPreview();
    select.addEventListener('change', startPreview);

    const cleanup = () => {
      if (previewStream) { previewStream.getTracks().forEach(t => t.stop()); previewStream = null; }
      preview.srcObject = null;
      overlay.classList.remove('open');
      select.removeEventListener('change', startPreview);
    };

    $('cam-picker-start').onclick = () => {
      const deviceId = select.value;
      const resolution = ($('cam-resolution') as HTMLSelectElement).value;
      const fps = parseInt(($('cam-fps') as HTMLSelectElement).value);
      cleanup();
      resolve({ deviceId, resolution, fps });
    };
    $('cam-picker-cancel').onclick = () => { cleanup(); resolve(null); };

    overlay.classList.add('open');
  });
}

function showCameraPiP(stream: MediaStream) {
  const pip = $('cam-pip');
  const video = pip.querySelector('video') as HTMLVideoElement;
  video.srcObject = stream;
  pip.classList.add('active');
}

function hideCameraPiP() {
  const pip = $('cam-pip');
  const video = pip.querySelector('video') as HTMLVideoElement;
...
let mobileDrawerOpen: 'left' | 'right' | null = null;

function initMobileUI() {
  const overlay = $('drawer-overlay');
  const leftDrawer = $('mobile-drawer-left');
  const rightDrawer = $('mobile-drawer-right');

  // Move widgets into mobile drawers
  const leftWidgets = document.querySelector('.left-widgets')!;
  const rightWidgets = document.querySelector('.right-widgets')!;

  // Clone channel-widget and user-widget into left drawer
  leftDrawer.appendChild(leftWidgets.querySelector('.channel-widget')!.cloneNode(true));
  leftDrawer.appendChild(leftWidgets.querySelector('.user-widget')!.cloneNode(true));

  // Clone online-widget into right drawer
  rightDrawer.appendChild(rightWidgets.querySelector('.online-widget')!.cloneNode(true));

  // Sync IDs for drawer copies (prefix with m-)
  leftDrawer.querySelector('#channel-list')?.setAttribute('id', 'm-channel-list');
  leftDrawer.querySelector('#add-channel-btn')?.setAttribute('id', 'm-add-channel-btn');
  leftDrawer.querySelector('#voice-status-bar')?.setAttribute('id', 'm-voice-status-bar');
  leftDrawer.querySelector('#voice-status-channel')?.setAttribute('id', 'm-voice-status-channel');
  leftDrawer.querySelector('#voice-disconnect-btn')?.setAttribute('id', 'm-voice-disconnect-btn');
  leftDrawer.querySelector('#user-avatar')?.setAttribute('id', 'm-user-avatar');
  leftDrawer.querySelector('#user-name-display')?.setAttribute('id', 'm-user-name-display');
  leftDrawer.querySelector('#user-role-display')?.setAttribute('id', 'm-user-role-display');
  leftDrawer.querySelector('#logout-btn')?.setAttribute('id', 'm-logout-btn');
  rightDrawer.querySelector('#online-list')?.setAttribute('id', 'm-online-list');
  rightDrawer.querySelector('#online-count')?.setAttribute('id', 'm-online-count');

  // Setup drawer user info
  const mUserName = leftDrawer.querySelector('#m-user-name-display');
  const mUserRole = leftDrawer.querySelector('#m-user-role-display');
  if (mUserName) mUserName.textContent = identity;
  if (mUserRole) mUserRole.textContent = role === 'admin' ? '管理员' : '成员';
  if (role === 'admin') {
    const mAddBtn = leftDrawer.querySelector('#m-add-channel-btn') as HTMLElement | null;
    if (mAddBtn) mAddBtn.style.display = '';
  }

  // Mobile drawer toggles
  $('mobile-menu-btn').addEventListener('click', () => toggleMobileDrawer('left'));
  $('mobile-online-btn').addEventListener('click', () => toggleMobileDrawer('right'));
  overlay.addEventListener('click', closeMobileDrawers);

  // Mobile drawer event listeners
  const mLogout = leftDrawer.querySelector('#m-logout-btn');
  if (mLogout) mLogout.addEventListener('click', logout);
  const mAvatar = leftDrawer.querySelector('#m-user-avatar');
  if (mAvatar) mAvatar.addEventListener('click', pickAvatar);
  const mAddBtn = leftDrawer.querySelector('#m-add-channel-btn');
  if (mAddBtn) mAddBtn.addEventListener('click', () => { closeMobileDrawers(); $('add-channel-modal').classList.add('open'); });
  const mDisc = leftDrawer.querySelector('#m-voice-disconnect-btn');
  if (mDisc) mDisc.addEventListener('click', async () => { closeMobileDrawers(); await leaveCurrentChannel(); renderChannels(); showEmptyState(); });
}

function toggleMobileDrawer(side: 'left' | 'right') {
  const overlay = $('drawer-overlay');
  if (mobileDrawerOpen === side) {
    closeMobileDrawers();
    return;
  }
  closeMobileDrawers();
  mobileDrawerOpen = side;
  overlay.classList.add('open');
  if (side === 'left') {
    $('mobile-drawer-left').classList.add('open');
    syncMobileChannels();
    syncMobileUserWidget();
  } else {
    $('mobile-drawer-right').classList.add('open');
    syncMobileOnline();
  }
}
...
function closeMobileDrawers() {
  ...
  $('mobile-drawer-left').classList.remove('open');
  $('mobile-drawer-right').classList.remove('open');
}

// Sync mobile drawer content from main widgets
function syncMobileChannels() {
  const src = $('channel-list');
  const dst = document.getElementById('m-channel-list');
  if (src && dst) {
    dst.innerHTML = src.innerHTML;
    // Re-bind double-click for channel join
    dst.querySelectorAll('.channel-item').forEach((el, idx) => {
      el.addEventListener('dblclick', () => {
        const ch = channels[idx];
        if (ch) { closeMobileDrawers(); joinChannel(String(ch.id), ch.name); }
      });
    });
  }
}
...
function syncMobileOnline() {
  const src = $('online-list');
  const dst = document.getElementById('m-online-list');
  const srcCount = $('online-count');
  const dstCount = document.getElementById('m-online-count');
  if (src && dst) dst.innerHTML = src.innerHTML;
  if (srcCount && dstCount) dstCount.textContent = srcCount.textContent ?? '0';
}

function syncMobileUserWidget() {
  const mUserAvatar = document.getElementById('m-user-avatar');
  if (mUserAvatar) {
    if (myAvatar) mUserAvatar.innerHTML = `<img src="${myAvatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
    else { mUserAvatar.innerHTML = ''; mUserAvatar.textContent = identity.charAt(0).toUpperCase(); }
  }
  const mVsBar = document.getElementById('m-voice-status-bar');
  const mVsCh = document.getElementById('m-voice-status-channel');
  if (mVsBar) {
    if (currentChannelId && currentChannelName) { mVsBar.classList.add('active'); if (mVsCh) mVsCh.textContent = currentChannelName; }
    else mVsBar.classList.remove('active');
  }
}
...
function updateMobileHeader() {
  ...
  if (currentChannelName) title.textContent = currentChannelName;
  else title.textContent = 'RaySpeaki';
}
...
// Camera PiP close
$('cam-pip-close').addEventListener('click', () => $('cam-pip').classList.remove('active'));

// Keep screen on during voice (mobile)
if (isMobile && 'wakeLock' in navigator) {
  let wakeLock: any = null;
  const origJoin = joinChannel;
  // Keep screen on when in channel
  setInterval(async () => {
    if (currentChannelId && !wakeLock) {
      try { wakeLock = await (navigator as any).wakeLock.request('screen'); } catch {}
    } else if (!currentChannelId && wakeLock) {
