...
class UnifiedPlan extends HandlerInterface {
  ...
  Future<HandlerSendResult> send(HandlerSendOptions options) async {
    _assertSendRirection();
    debugPrint('[UP-send] START track=${options.track.kind} id=${options.track.id}');
    ...
    debugPrint('[UP-send] 1. RtpParameters.copy...');
    ...
    debugPrint('[UP-send] 2. getNextMediaSectionIdx...');
    MediaSectionIdx mediaSectionIdx = _remoteSdp.getNextMediaSectionIdx();

    debugPrint('[UP-send] 3. addTransceiver...');
    ...
    debugPrint('[UP-send] 3. addTransceiver OK');

    RTCSessionDescription offer;
    try {
      debugPrint('[UP-send] 4. createOffer...');
      offer = await _pc!.createOffer({});
      debugPrint('[UP-send] 4. createOffer OK, sdp len=${offer.sdp?.length}');
    } catch (e) {
      debugPrint('[UP-send] 4. createOffer FAILED: $e');
      rethrow;
    }
    ...
    debugPrint('[UP-send] 5. _transportReady=$_transportReady');
    if (!_transportReady) {
      debugPrint('[UP-send] 5a. _setupTransport...');
      await _setupTransport(
        localDtlsRole: DtlsRole.server,
        localSdpObject: localSdpObject,
      );
      debugPrint('[UP-send] 5b. _setupTransport OK');
    }
    ...
    debugPrint('[UP-send] 6. setLocalDescription...');
    await _pc!.setLocalDescription(offer);
    debugPrint('[UP-send] 6. setLocalDescription OK');

    if (!kIsWeb) {
      debugPrint('[UP-send] 7. getTransceivers...');
      final transceivers = await _pc!.getTransceivers();
      debugPrint('[UP-send] 7. getTransceivers OK, count=${transceivers.length}');
    ...
    debugPrint('[UP-send] 8. localId=$localId');
    ...
    debugPrint('[UP-send] 9. _remoteSdp.send...');
    _remoteSdp.send(
    ...
    debugPrint('[UP-send] 9. _remoteSdp.send OK');

    RTCSessionDescription answer = RTCSessionDescription(
      _remoteSdp.getSdp(),
      'answer',
    );
    debugPrint('[UP-send] 10. answer SDP len=${answer.sdp?.length}');
    debugPrint('[UP-send] 10. answer SDP first 500: ${answer.sdp?.substring(0, answer.sdp!.length > 500 ? 500 : answer.sdp!.length)}');

    try {
      debugPrint('[UP-send] 11. setRemoteDescription...');
      await _pc!.setRemoteDescription(answer);
      debugPrint('[UP-send] 11. setRemoteDescription OK');
    } catch (e) {
      debugPrint('[UP-send] 11. setRemoteDescription FAILED: $e');
      ...
      debugPrint('[UP-send] 11-fallback. setRemoteDescription retry...');
      await _pc!.setRemoteDescription(answer);
      debugPrint('[UP-send] 11-fallback. setRemoteDescription retry OK');
    }

    debugPrint('[UP-send] 12. storing transceiver, done!');
    // Store in the map.
    _mapMidTransceiver[localId] = transceiver;
