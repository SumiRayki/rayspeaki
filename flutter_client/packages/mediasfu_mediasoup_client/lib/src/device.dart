...
class Device {
  ...
  Future<void> load({
    ...
    try {
      ...
      // 补充 native 缺失但 router 支持的视频 codec（如 H264）
      // 部分设备/模拟器的 getNativeRtpCapabilities 不含 H264，
      // 但 WebRTC 软解码器可以处理，这里确保 extended capabilities 包含它们
      final nativeMimeTypes = nativeRtpCapabilities.codecs
          .map((c) => c.mimeType.toLowerCase())
          .toSet();
      for (final routerCodec in routerRtpCapabilities.codecs) {
        final mime = routerCodec.mimeType.toLowerCase();
        if (mime.startsWith('video/') && !nativeMimeTypes.contains(mime)) {
          nativeRtpCapabilities.codecs.add(routerCodec);
          nativeMimeTypes.add(mime);
        }
      }
